...
public class ImmutableEvoInfo extends EvoInfo {
    ...
    public void save(ImmutableResearchObject previousRO) {
    	super.save();
        ...
        try {
            ...
            if (previousRO != null) {
            	model.removeAll(model.getIndividual(researchObject.getUri().toString()), 
            					PROV.wasRevisionOf, null);
            ...
            switch(evoType){
	            case ARCHIVE: getResearchObject().postEvent(new ROArchiveAfterCreateEvent(getResearchObject()));
	            	break;
	            case SNAPSHOT: getResearchObject().postEvent(new ROSnapshotAfterCreateEvent(getResearchObject()));
	            	break;
	            case LIVE: // no event is expected here
	            	break;
	            case FORK: // no event is expected here
	            	break; 
            }
    ...
    public void load() {
        ...
        try {
            ...
            } else if (ro.hasRDFType(ROEVO.ForkedRO)){
            	evoType = EvoType.FORK;
            } else {
