...
public class Annotation extends AggregatedResource {
	...
	public Annotation copy(Builder builder, EvoBuilder evoBuilder, ResearchObject researchObject)
		...
		for (Thing target : getAnnotated()) {
			...
			if (getResearchObject().getAggregatedResources().containsKey(target.getUri())) {
				...
				URI researchObjectUri = researchObject.getUri();
				String aggregatedResourceRawPath = getResearchObject().getAggregatedResources().get(target.getUri()).getRawPath();
				
				targetUri = researchObjectUri.resolve(aggregatedResourceRawPath);
				
				/*targetUri = researchObject.getUri().resolve(
						getResearchObject().getAggregatedResources().get(target.getUri())
								.getRawPath());*/
			} else {
				// if annotation applies directly to RO
				if(target.getUri().equals(getResearchObject().getUri())){
					// change targetURI to new RO
					targetUri = researchObject.getUri();
				} else {
	...
	public boolean containsTripple(String subject, String property, String object){
		
		String cmd = "ASK { ?subject ?property ?object }";
    	
		ParameterizedSparqlString paramSparql = new ParameterizedSparqlString();
		paramSparql.setCommandText(cmd);
		
		if(subject != null)
			paramSparql.setParam("subject", NodeFactory.createURI(subject));
		if(property != null)
			paramSparql.setParam("property", NodeFactory.createURI(property));
		if(object != null)
			paramSparql.setParam("object", NodeFactory.createURI(object));
		
		Query query = QueryFactory.create(paramSparql.asQuery()) ;
		boolean started = body.beginTransaction(ReadWrite.READ);
		boolean result;
		if (body.getModel() != null){	
			//LOGGER.debug("contains triple query " + query.toString() + " body "+body +" body model "+body.getModel());
			QueryExecution qexec = QueryExecutionFactory.create(query, body.getModel());
			result = qexec.execAsk();
			qexec.close();
		}
		else {
			LOGGER.debug("body is null" + body);
			result=false;
		}
		body.endTransaction(started);
		return result;
    }
